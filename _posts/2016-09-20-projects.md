...
I'm using Amazon Web Services to manage Boghop.com. I bought the domain name through AWS, and I use Route 53 to manage DNS. For the server, I created an EC2 instance, which hosts one of my web publishing apps for creates and updates. The EC2 server uses the Nginx web server. My app copies files to an S3 bucket. I use CloudFront for edge caching. If an HTML file is not cached, then it's retrieved from S3.
